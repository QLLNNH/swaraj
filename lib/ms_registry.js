...
module.exports = class MS_Registry extends Events {
    ...
    async init_task() {
        this.init_update_timer();

        if (Array.isArray(this.watched)) {
            this.init_watcher();
            ...
            try {
                for (let service of this.instance.services) {
                    ...
                    const raw = await this.model.updateOne(
                        { service, host: this.instance.host, port: this.instance.port },
                        { $set: { u_ts: new Date() } }
                    );
    ...
    insert(instance) {
        ...
        socket.on('connect', () => {
            ...
            this.instances.set(instance._id, {
                _id: instance._id,
            ...
            logger.info({
                ...
                grade: 'INFO',
                message: `insert ${ instance.service }:${ instance.host }:${ instance.port }`
            });
        });
    ...
    delete(id) {
        if (this.instances.has(id)) {
            ...
            logger.info({
                ts: new Date().toISOString(),
                grade: 'INFO',
                message: `delete ${ instance.service }:${ instance.host }:${ instance.port }`
            });
        }
    }
}