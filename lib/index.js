...
class Swaraj {
    ...
    init_registry({ host, port, routes, watched }) {
        ...
        if (typeof host !== 'string' || host.split('.').length !== 4)
            host = ip_collector.get_inner_ip();
        ...
        this.registry = new MS_Registry(this.mongo.model, {
            host,
            port,
            routes,
            watched,
    ...
    get_route({ service, method, path }) {
        ...
        if (! routes) throw Result.of(201, { service });

        if (! routes[method]) throw Result.of(202, { method });

        const route = routes[method].find((item) => item.path.test(path));
        if (! route) throw Result.of(203, { path });
    ...
    async rpc(bank) {
        ...
        else return this.distributer.rpc_on_http(bank);
    }
}
